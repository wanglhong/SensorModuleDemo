          ...
          >
            <template v-slot:toolbar>
              <lay-button
                size="sm"
                type="primary"
                @click="displayFromLay('新增', customsClearanceInfoDto())"
              >
            ...
            <template v-slot:customsDeclarationInfo="{ data }">
              {{ data.customsDeclarationInfo?.customsDeclarationName }}
            </template>
            <template v-slot:inspectionOrganization="{ data }">
              {{ data.inspectionOrganization?.organizationName }}
            </template>
...
import FromLay from '@/views/customInfo/customsClearanceInfo/FormLay.vue'
import SearchBox from '@/views/customInfo/customsClearanceInfo/SearchBox.vue'
import {
  list,
  remove,
  removeByIdList,
} from '@/api/module/customsClearanceInfo.js'
import { customsClearanceInfoDto } from '@/model/ModelDto.js'
import {
  add as fileAdd,
  delFile as fileRemove,
} from '@/api/module/clearanceInfoBusinessFile.js'
...
const columns = [
  ...
  {
    title: '报关信息',
    key: 'customsDeclarationInfoId',
    customSlot: 'customsDeclarationInfo',
    ellipsisTooltip: true,
    align: 'center',
  },
  {
    title: '清关名称',
    key: 'customsClearanceName',
    ellipsisTooltip: true,
    align: 'center',
  },
  {
  ...
  },
  {
    ...
    customSlot: 'inspectionOrganization',
    ellipsisTooltip: true,
    align: 'center',
  },
  {
...
function delFile(file, isUpdate) {
  const param = {
    ...
    clearanceInfoId: currenId.value,
  }
  fileRemove(param)
    .then(({ success, code, msg, data }) => {
      if (success) {
...
// 文件上传提交
function toSubmit() {
  uploadForm.clearanceInfoId = currenId.value
  fileAdd(uploadForm)
    .then(({ success, code, msg, data }) => {
      if (success) {
        layer.msg(msg, { icon: 1 })
