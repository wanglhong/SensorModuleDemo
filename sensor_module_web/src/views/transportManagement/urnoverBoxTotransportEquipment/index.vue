          ...
          >
            <template v-slot:toolbar>
              <lay-button
                size="sm"
                type="primary"
                @click="displayFromLay('新增', goodsToTurnoverBoxDto())"
              >
            ...
            <template v-slot:turnoverBox="{ data }">
              {{ data.turnoverBox.turnoverBoxName }}
            </template>
            <template v-slot:goods="{ data }">
              {{ data.goods.goodsName }}
            </template>
            <template v-slot:goodsToBoxUser="{ data }">
              {{ data.goodsToBoxUser.userName }}
            </template>
...
import FromLay from '@/views/transportManagement/urnoverBoxTotransportEquipment/FormLay.vue'
import SearchBox from '@/views/transportManagement/urnoverBoxTotransportEquipment/SearchBox.vue'
import {
  list,
  remove,
  removeByIdList,
} from '@/api/module/goodsToTurnoverBox.js'
...
const columns = [
  ...
  {
    title: '货物装箱操作员ID',
    key: 'goodsToBoxUserId',
    customSlot: 'goodsToBoxUser',
    ellipsisTooltip: true,
    align: 'center',
  },
...
function loadDataSource() {
  ...
  list({ modelDto: modelDto, page: page })
    .then(({ success, code, msg, data }) => {
      ...
      loading.value = false
      if (success) {
        dataSource.value = data.data.filter(
          (item) =>
            item.transportInfoId !== null && item.transportInfoId !== undefined
        )
