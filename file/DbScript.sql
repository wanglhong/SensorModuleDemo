...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '基础附件表';

-- ------------------------------------------------------------
-- sm_clearance_info_business_file | 清关信息与附件多对多关联表
-- ------------------------------------------------------------
DROP TABLE IF EXISTS sm_clearance_info_business_file;
CREATE TABLE IF NOT EXISTS sm_clearance_info_business_file
(
    id                BIGINT       NOT NULL COMMENT '主键ID',
    create_time       DATETIME     NULL COMMENT '创建时间',
    create_user_id    BIGINT       NULL COMMENT '创建者ID',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '清关信息与附件多对多关联表';
...
(
    ...
    data_dept_id                BIGINT       NULL COMMENT '数据所属部门ID',
    is_delete                   INT          NULL COMMENT '逻辑删除（1：存在。-1：逻辑删除）',
    audit_name                  VARCHAR(255) NULL COMMENT '审批名称',
    ...
    customs_clearance_info_id   BIGINT       NULL COMMENT '清关信息ID',
    audit_state                 INT          NULL COMMENT '审批状态',
    remark                      VARCHAR(255) NULL COMMENT '备注',
    user_id_of_audit            BIGINT       NULL COMMENT '审批人ID',
    PRIMARY KEY (id)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT = '海关审核表';
...
(
    ...
    data_dept_id                BIGINT         NULL COMMENT '数据所属部门ID',
    is_delete                   INT            NULL COMMENT '逻辑删除（1：存在。-1：逻辑删除）',
    customs_clearance_name      VARCHAR(255)   NULL COMMENT '清关信息名称',
    transport_info_id           BIGINT         NULL COMMENT '运输信息ID',
    customs_declaration_info_id BIGINT         NULL COMMENT '报关信息ID',
    customs_clearance_number    VARCHAR(255)   NULL COMMENT '清关单号',
    inspection_organization_id  BIGINT         NULL COMMENT '检验检疫机构ID',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '海关清关信息表';
...
(
    ...
    data_dept_id               BIGINT         NULL COMMENT '数据所属部门ID',
    is_delete                  INT            NULL COMMENT '逻辑删除（1：存在。-1：逻辑删除）',
    customs_declaration_name   VARCHAR(255)   NULL COMMENT '报关信息名称',
    transport_info_id          BIGINT         NULL COMMENT '运输信息ID',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '海关报关信息表';

-- ------------------------------------------------------------
-- sm_declaration_info_business_file | 报关信息与附件多对多关联表
-- ------------------------------------------------------------
DROP TABLE IF EXISTS sm_declaration_info_business_file;
CREATE TABLE IF NOT EXISTS sm_declaration_info_business_file
(
    id                  BIGINT       NOT NULL COMMENT '主键ID',
    create_time         DATETIME     NULL COMMENT '创建时间',
    create_user_id      BIGINT       NULL COMMENT '创建者ID',
    ...
    data_dept_id        BIGINT       NULL COMMENT '数据所属部门ID',
    is_delete           INT          NULL COMMENT '逻辑删除（1：存在。-1：逻辑删除）',
    declaration_info_id BIGINT       NULL COMMENT '报关信息ID',
    business_file_id    BIGINT       NULL COMMENT '附件ID',
    remark              VARCHAR(255) NULL COMMENT '备注',
    PRIMARY KEY (id)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT = '报关信息与附件多对多关联表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '货物信息表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '物联网设备信息表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '运输设备信息表';
...
(
    ...
    data_dept_id            BIGINT       NULL COMMENT '数据所属部门ID',
    is_delete               INT          NULL COMMENT '逻辑删除（1：存在。-1：逻辑删除）',
    transport_info_name     VARCHAR(255) NULL COMMENT '运输名称',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '运输信息表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '运输信息-周转箱-货物关联表（1*N*M）';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '周转箱信息表';

-- ------------------------------------------------------------
-- sm_gps_info | GPS定位信息表
-- ------------------------------------------------------------
DROP TABLE IF EXISTS sm_gps_info;
CREATE TABLE IF NOT EXISTS sm_gps_info
(
    id                BIGINT         NOT NULL COMMENT '主键ID',
    create_time       DATETIME       NULL COMMENT '创建时间',
    create_user_id    BIGINT         NULL COMMENT '创建者ID',
    ...
    longitude         DECIMAL(19, 4) NULL COMMENT '经度',
    latitude          DECIMAL(19, 4) NULL COMMENT '纬度',
    speed             VARCHAR(255)   NULL COMMENT '速度',
    direction         VARCHAR(255)   NULL COMMENT '方向',
    altitude          VARCHAR(255)   NULL COMMENT '海拔',
    PRIMARY KEY (id)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT = 'GPS定位信息表';

-- ------------------------------------------------------------
-- sm_user_proximity_info | 人物靠近信息表
-- ------------------------------------------------------------
DROP TABLE IF EXISTS sm_user_proximity_info;
CREATE TABLE IF NOT EXISTS sm_user_proximity_info
(
    id                BIGINT   NOT NULL COMMENT '主键ID',
    create_time       DATETIME NULL COMMENT '创建时间',
    create_user_id    BIGINT   NULL COMMENT '创建者ID',
    ...
    is_delete         INT      NULL COMMENT '逻辑删除（1：存在。-1：逻辑删除）',
    transport_info_id BIGINT   NULL COMMENT '运输信息ID',
    proximity_date    DATETIME NULL COMMENT '靠近时间',
    gps_info_id       BIGINT   NULL COMMENT 'GPS信息ID',
    monitor_info_id   BIGINT   NULL COMMENT '监控信息ID',
    PRIMARY KEY (id)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT = '人物靠近信息表';

-- ------------------------------------------------------------
-- sm_video_info | 视频信息表
-- ------------------------------------------------------------
DROP TABLE IF EXISTS sm_video_info;
CREATE TABLE IF NOT EXISTS sm_video_info
(
    id                 BIGINT   NOT NULL COMMENT '主键ID',
    create_time        DATETIME NULL COMMENT '创建时间',
    create_user_id     BIGINT   NULL COMMENT '创建者ID',
    ...
    is_delete          INT      NULL COMMENT '逻辑删除（1：存在。-1：逻辑删除）',
    transport_info_id  BIGINT   NULL COMMENT '运输信息ID',
    base_attachment_id BIGINT   NULL COMMENT '基础附件ID',
    PRIMARY KEY (id)
) ENGINE = InnoDB
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '系统组织（公司）表';
